import { Box, Center, FormControl, FormLabel, HStack, IconButton, Input, notificationService, Select, SelectContent, SelectIcon, SelectListbox, SelectOption, SelectOptionIndicator, SelectOptionText, SelectPlaceholder, SelectTrigger, SelectValue, VStack } from '@hope-ui/solid';
...
let AdminEditUserPage = () => {
    ...
    let completeProfile = () => {
        axios.put(apiUrl + "/users/", details, { headers: { "Authorization": "Bearer " + authState.authenticationToken } }).then((response) => {
            if (response.data.error) {
                notificationService.show({
                    title: 'Error',
                    description: 'Failed to update users profile.',
                    status: 'danger',
                    duration: 3000,
                });
            } else {
                notificationService.show({
                    title: 'Success',
                    description: 'Updated users profile successfully.',
                    status: 'success',
                    duration: 3000,
                });
    ...
    return (
        <Center w="100%" h="100%" roundedTop={"$2xl"}>
            {loading() && <VStack>
            ...
            {!loading() && <VStack>
                {stage() === 0 && (
                    <Box class="animate-fade-in" color="black">
                        Let's begin with their basic details.
                    </Box>
                )}

                {stage() === 2 && (
                    <Box class="animate-fade-in" color="black">
                        Let's get their business details.
                    </Box>
                )}

                {stage() === 4 && (
                    <Box class="animate-fade-in" color="black">
                        Let's get their handles. You can leave stuff out here if you don't
                        have it.
                    </Box>
                )}
                ...
                {stage() === 6 && (
                    <Box class="animate-fade-in" color="black">
                        Let's get their bank details.
                    </Box>
                )}

                {stage() === 8 && (
                    <Box class="animate-fade-in" color="black">
                        Let's get their location details.
                    </Box>
                )}

                {stage() === 10 && (
                    <Box class="animate-fade-in" color="black">
                        You have successfully edited their profile.
                    </Box>
                )}
